...
export async function matchResultRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: MatchIdParams }>(
    "/match/:match_id",
    {
      schema: {
    ...
    async (request, reply) => {
      ...
      const isHomeTeamMember = await matchResultsService.isTeamMember(
        userId,
        match.home_team_id,
      );
      const isAwayTeamMember = await matchResultsService.isTeamMember(
        userId,
        match.away_team_id,
      );
      const isLeagueManager = await matchResultsService.isLeagueManager(
        userId,
        match.league_id,
      );
  ...
  fastify.get<{ Params: SubmissionIdParams }>(
    ...
    async (request, reply) => {
      ...
      // Get match details to check permissions
      const match = await matchResultsService.getMatchDetails(
        submission.match_id,
      );
      if (!match) {
        throw new NotFoundError("Match");
  ...
  fastify.post(
    ...
    async (
      request: FastifyRequest<{ Body: CreateMatchResultSubmissionBody }>,
      reply: FastifyReply,
    ) => {
      const userId = request.user.id.toString();
      const data = request.body;

      // Guard: check league is writable via match
      const match = await matchResultsService.getMatchDetails(data.match_id);
      if (match) await ensureLeagueWritable(db, match.league_id);
  ...
  fastify.patch<{
    ...
    Body: UpdateMatchResultSubmissionBody;
  }>(
    "/:id",
    {
      schema: {
    ...
    async (request, reply) => {
      ...
      await ensureLeagueWritable(db, match.league_id);
      ...
      if (!isLeagueManager) {
        reply
          .code(403)
          .send({ error: "Only league managers can update submission status" });
        return;
      }

      const updated = await matchResultsService.updateSubmissionStatus(
        id,
        data,
      );
  ...
  fastify.delete<{ Params: SubmissionIdParams }>(
    "/:id",
    {
      schema: {
        params: submissionIdParamsSchema,
