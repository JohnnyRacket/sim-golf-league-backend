...
export async function matchRoutes(fastify: FastifyInstance) {
  ...
  fastify.get(
    ...
    async (
      ...
      const mapped = enhancedMatches.map((match) => ({
        id: match.id,
        match_date: match.match_date,
        status: match.status,
  ...
  fastify.get(
    ...
    async (
      ...
      return matchesService.getUserMatchSummaries(user_id);
    },
  );
  ...
  fastify.get(
    ...
    async (request, reply) => {
      const userId = request.user.id.toString();
      return matchesService.getUserMatchSummaries(userId);
    },
  );
  ...
  fastify.post<{ Body: CreateMatchBody }>(
    ...
    async (request, reply) => {
      ...
      const teamVerification = await matchesService.verifyTeamsForMatch(
        home_team_id,
        away_team_id,
        league_id,
      );
      ...
      if (!teamVerification) {
        throw new ValidationError(
          "Invalid team selection. Teams must exist, be in the specified league, and cannot be the same.",
        );
      }
  ...
  // Update match
  fastify.put<{ Params: { id: string }; Body: UpdateMatchBody }>(
    "/:id",
    {
      preHandler: checkRole(["admin", "manager"]),
      schema: {
    ...
    async (request, reply) => {
      ...
      if (!isAdmin) {
        isAuthorized = await matchesService.isLeagueManager(
          id,
      ...
      if (!isAuthorized) {
        reply
          .code(403)
          .send({ error: "You are not authorized to update this match" });
        return;
      }
      ...
      if (updateData.simulator_settings)
        data.simulator_settings = updateData.simulator_settings;
      if (updateData.game_format)
        data.game_format = updateData.game_format as GameFormatType;
      if (updateData.match_format)
  ...
  fastify.post<{ Params: { id: string }; Body: MatchGameBody }>(
    "/:id/games",
    {
    ...
    async (request, reply) => {
      reply.code(410).send({
        ...
        message:
          "Games are now stored directly in the match record. Use the submit match results endpoint.",
      });
    },
  );
  ...
  fastify.put<{ Params: { id: string }; Body: SubmitMatchResultsBody }>(
    "/:id/result",
    {
      preHandler: checkRole(["admin", "manager"]),
      schema: {
  ...
  fastify.put<{
    ...
    "/league/:league_id/day",
    {
      ...
      schema: {
        description:
          "Bulk update matches for a specific day of a league (admin or manager only)",
        tags: ["matches"],
    ...
    async (request, reply) => {
      ...
      if (!isAdmin) {
        ...
        isAuthorized =
          manager.user_id.toString() === request.user.id.toString();
      }
      ...
      if (result.matches_updated === 0) {
        throw new NotFoundError(
          "No matches found for the specified date in this league",
        );
      }
