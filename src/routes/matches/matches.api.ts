...
export async function matchRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: CreateMatchBody }>('/', {
    ...
    if (!teamVerification) {
      throw new ValidationError('Invalid team selection. Teams must exist, be in the specified league, and cannot be the same.');
    }
  ...
  fastify.put<{
    ...
    if (result.matches_updated === 0) {
      throw new NotFoundError('No matches found for the specified date in this league');
    }
