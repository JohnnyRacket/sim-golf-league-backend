...
export async function handicapRoutes(fastify: FastifyInstance) {
  ...
  fastify.put<{
    Params: { leagueId: string; userId: string };
    Body: SetHandicapBody;
  }>(
    "/leagues/:leagueId/players/:userId",
    {
      schema: {
    ...
    async (request, reply) => {
      ...
      await ensureLeagueWritable(db, leagueId);
      ...
      const result = await handicapsService.setPlayerHandicap({
        ...
        effectiveDate: request.body.effective_date
          ? new Date(request.body.effective_date)
          : undefined,
      });
  ...
  fastify.get<{ Params: { leagueId: string; userId: string } }>(
    "/leagues/:leagueId/players/:userId/history",
    {
      schema: {
        params: leagueUserIdParamsSchema,
        response: {
  ...
  fastify.put<{ Params: { teamId: string }; Body: SetTeamHandicapBody }>(
    "/teams/:teamId",
    {
      schema: {
        params: teamIdParamsSchema,
