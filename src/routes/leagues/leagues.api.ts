...
export async function leagueRoutes(fastify: FastifyInstance) {
  ...
  // Get all leagues
  fastify.get<{
    Querystring: { location_id?: string } & PaginationQuery;
  }>(
    "/",
    {
      schema: {
    ...
    async (request) => {
      const { location_id, page, limit: queryLimit } = request.query;
      const leagues = await leaguesService.getAllLeagues(location_id);

      if (page || queryLimit) {
        const { page: p, limit: l, offset } = parsePagination(request.query);
        return buildPaginatedResult(
          leagues.slice(offset, offset + l),
          leagues.length,
          p,
          l,
        );
  ...
  fastify.post<{ Body: CreateLeagueBody }>(
    ...
    async (request, reply) => {
      ...
      const isLocationManager = await leaguesService.isUserLocationManager(
        location_id,
        userId,
      );
  ...
  fastify.put<{ Params: { id: string }; Body: UpdateLeagueBody }>(
    "/:id",
    {
      preHandler: checkRole(["manager"]),
      schema: {
    ...
    async (request, reply) => {
      ...
      if (!isAuthorized) {
        return reply
          .code(403)
          .send({ error: "You are not authorized to update this league" });
      }
  ...
  fastify.delete<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!isAuthorized) {
        return reply
          .code(403)
          .send({ error: "You are not authorized to delete this league" });
      }
  ...
  fastify.post<{ Params: { id: string }; Body: UpdateLeagueMemberBody }>(
    "/:id/members",
    {
    ...
    async (request, reply) => {
      ...
      const isAuthorized = await leaguesService.isUserLeagueManager(
        id,
      ...
      if (!isAuthorized) {
        return reply
          .code(403)
          .send({ error: "Not authorized to manage league members" });
      }
  ...
  fastify.put<{
    ...
    async (request, reply) => {
      ...
      const member = await leaguesService.updateLeagueMemberRole(
        memberId,
        role,
      );
      if (!member) {
  ...
  fastify.get<{
    ...
    async (request, reply) => {
      ...
      if (!isAuthorized) {
        return reply
          .code(403)
          .send({ error: "Not authorized to view league requests" });
      }
  ...
  fastify.post<{ Body: CreateMembershipRequestBody }>(
    "/:id/join",
    {
      schema: {
    ...
    async (request, reply) => {
      ...
      if (requested_role === "manager") {
        const hasManagers = await leaguesService
          .getLeagueMembers(id)
          .then((members) => members.some((m) => m.role === "manager"));

        if (hasManagers) {
          const isOwner = await leaguesService.isUserLocationManager(
            league.location_id,
            userId,
          );
          if (!isOwner) {
            throw new ValidationError(
              "Only location owners can request manager role for established leagues",
            );
          }
        }
      }
      ...
      return reply
        .code(201)
        .send({ message: "Membership request created successfully" });
    },
  );
  ...
  fastify.post<{ Params: { id: string; requestId: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!isAuthorized) {
        return reply
          .code(403)
          .send({ error: "Not authorized to manage league requests" });
      }

      const success =
        await leaguesService.approveLeagueMembershipRequest(requestId);
      if (!success) {
  ...
  fastify.post<{ Params: { id: string; requestId: string } }>(
    ...
    async (request, reply) => {
      ...
      const success =
        await leaguesService.rejectLeagueMembershipRequest(requestId);
      if (!success) {
        throw new NotFoundError("Request");
      }
  ...
  fastify.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      const matchesCreated = await leaguesService.generateSchedule(
        id,
        teams,
        league.scheduling_format,
      );
